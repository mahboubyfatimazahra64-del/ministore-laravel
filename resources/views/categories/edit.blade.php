                ...
                <form action="{{ route('categories.update', $category) }}" method="POST">
                    ...
                    @method('PUT') <div class="mb-4">
                        ...
                        <input type="text" name="name" value="{{ $category->name }}" 
                               class="w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" required>
                    </div>

                    <div class="flex justify-end gap-2">
                        <a href="{{ route('categories.index') }}" class="px-4 py-2 text-gray-700 bg-gray-200 rounded-md hover:bg-gray-300">Annuler</a>
                        <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700">Mettre à jour</button>
                    </div>
                </form>
            </div>
        </div>
